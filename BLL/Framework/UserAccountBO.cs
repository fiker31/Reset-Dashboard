...
namespace BLL.Framework
{
    ...
    public class UserAccountBO : BaseEO
    {
        ...
        public override bool Save(DBDataContext db, ref EntValidationErrors validationErrors, string userAccountId)
        {
            if (DBAction == DBActionEnum.Insert || DBAction == DBActionEnum.Update)
            {
                ...
                if (validationErrors.Count == 0)
                {
                    ...
                    {
                        //Add
                        Id = new UserAccountData().Insert(db, UserAccountName, FirstName, LastName, Phone, Status, UserPassword, Shortcode, IsMerchant, userAccountId);
                    }
                    else
                    {
                        //Update
                        if (!new UserAccountData().Update(db, Id, UserAccountName, FirstName, LastName, Phone, Status, UserPassword, Shortcode, IsNewPassword, PasswordChangeDate, PasswordAttempt, IsLocked, IsMerchant, InsertUserId, EntryDate, userAccountId, Version))
                        {
        ...
        SystemSecurityDAC.GetSecurityParameterObject() : SystemSecurityDAC.GetEmpitySecurityParameterObject();
            ...
            {
                validationErrors.Add("Short Code is required.");
            }  //User Account Name is required.
            ...
            //User Account Name blank space.
            else if (UserAccountName.Contains(" "))
            {
                validationErrors.Add("Blank space is not allowed in Username ");
            }
        ...
        protected override void MapEntityToCustomProperties(IBaseEntity entity)
        {
            ...
            IsNewPassword = (bool)userAccount.IsNewPassword;
            PasswordAttempt = userAccount.PAsswordAttemptNo;
            IsLocked = userAccount.IsLocked;
            Shortcode = userAccount.Shortcode.ToString();
        }
